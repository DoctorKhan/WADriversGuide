...
- [x] Restored all flattened h3 subsection headings in the Bengali edition (was 30 h3s, now matches the English structure section-for-section across all 71 content sections) and added the ~45 missing en/bn h3 pairs in the unified edition. Machine translations for previously-untranslated headings follow the same first-pass status as the rest of the Bengali text.
- [x] Extended `scripts/verify.mjs` with cross-edition parity checks: per-section h2/h3 counts across all three files, TOC↔section coverage, visible-English detection in the Bengali quiz zone, unified en/bn heading balance, and output/↔docs/ drift detection.
